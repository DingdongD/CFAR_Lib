...
alpha_ca = ca_threhold(Pfa,NSlide); % CA门限系数
alpha_so = so_threhold(Pfa,NSlide); % SO门限系数
alpha_go = go_threhold(Pfa,NSlide); % GO门限系数
alpha_os = os_threhold(Pfa,NSlide,rate); % OS门限系数

% 信噪比设置
snr_db = 1 : 1 : 35; % 信噪比分贝区间
snr = 10 .^ (snr_db / 10); % 功率转化
monte_num = 1e1; % 蒙特卡洛仿真次数

% 检测矩阵初始化
Pd_CA = zeros(1, length(snr)); % CA-CFAR
Pd_SO = zeros(1, length(snr)); % SO-CFAR
Pd_GO = zeros(1, length(snr)); % GO-CFAR
Pd_OS = zeros(1, length(snr)); % OS-CFAR
...
for snr_id = 1 : length(snr)
    ...
    for monte_id = 1 : monte_num
        ...
        for i = 1 : target_num % 生成特定snr下的多目标
            ...
            clutter_data(target_idx(i)) = clutter_power * snr(snr_id);
        end
    ...
    % 统计每个snr下的目标检测概率
    Pd_CA(1, snr_id) = ca_count / monte_num / target_num; 
    Pd_GO(1, snr_id) = go_count / monte_num / target_num;
    Pd_SO(1, snr_id) = so_count / monte_num / target_num;
    Pd_OS(1, snr_id) = os_count / monte_num / target_num;
    
end
toc

figure(1);
plot(snr_db, Pd_CA, 'Color','#FFD700','LineWidth',2.0,'Marker','d');hold on;
plot(snr_db, Pd_GO, 'Color','#483D8B','LineWidth',2.0,'Marker','+');hold on;
plot(snr_db, Pd_SO, 'Color','#FF1493','LineWidth',2.0,'Marker','*');hold on;
plot(snr_db, Pd_OS, 'Color','#8B008B','LineWidth',2.0,'Marker','s');hold on;
